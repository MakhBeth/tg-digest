...
export function buildDigestPrompt(args: { groupTitle: string; formatted: string; profile: string; truncated: boolean }) {
  ...
  return {
    system: SYSTEM,
    user: `Riassumi la conversazione del gruppo "${args.groupTitle}". Evidenzia: thread principali, decisioni, link e risorse condivise.${profilePart}${truncPart}\n\nMessaggi:\n${args.formatted}`,
  }
}
